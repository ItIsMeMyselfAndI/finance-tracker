...
from frontend.widgets import DatePicker
...
class EditPageView(View):
    ...
    def _create_header(self):
        self.header = Header(
            master=self, fg_color=EditStyles.HEADER_SECTION_FG_COLOR, corner_radius=0
        )
    ...
    def _create_tabs(self):
        ...
        self.tabs.grid(
            row=1, column=0, padx=BaseStyles.PAD_3, pady=(BaseStyles.PAD_4, 0)
        )
...
class TransactionForm(ctk.CTkFrame):
    ...
    def _create_sections(self):
        self.top_section = ctk.CTkFrame(
            master=self, fg_color=EditStyles.FORM_TOP_SECTION_FG_COLOR, corner_radius=0
        )
        self.top_section.pack(fill="both", pady=(BaseStyles.PAD_4, 0))

        self.mid_section = ctk.CTkFrame(
            master=self, fg_color=EditStyles.FORM_MID_SECTION_FG_COLOR, corner_radius=0
        )
        self.mid_section.pack(fill="both", pady=(BaseStyles.PAD_4, 0))

        self.bot_section = ctk.CTkFrame(
            master=self, fg_color=EditStyles.FORM_BOT_SECTION_FG_COLOR, corner_radius=0
        )
    ...
    def _create_top_section_content(self):
        ...
        self.transactionLabel.grid(
            ...
            column=0,
            sticky="w",
            padx=(BaseStyles.PAD_3, 0),
            pady=(0, BaseStyles.PAD_3),
        )
        self.transactionMenu.grid(
            row=1, column=0, sticky="w", padx=(BaseStyles.PAD_3, 0), pady=0
        )
    ...
    def _create_mid_section_content(self):
        ...
        self.descriptionEntry.grid(
            row=1, column=1, sticky="w", padx=BaseStyles.PAD_3, pady=0
        )
    ...
    def _create_bot_section_content(self):
        ...
        self.amountLabel.pack(
            anchor="w", padx=BaseStyles.PAD_3, pady=(0, BaseStyles.PAD_3)
        )
        self.amountEntry.pack(anchor="w", padx=BaseStyles.PAD_3, pady=0)

    def update_transaction_menu_options_per_type(self):
        transactions = self.model.t_man.repo.getTransactionsByType(
            self.model.user_id_var.get(), self.t_type
        )
        transaction_options = []
        for t in transactions:
            formatted_t = f" {t.t_id} | {t.t_date} | {t.t_category} | {t.t_amount} | {t.t_description} "
            formatted_t = (
                f"{formatted_t[:60]}..." if len(formatted_t) > 60 else formatted_t
            )
...
class Tabs(ctk.CTkFrame):
    def __init__(self, form_per_transaction_type, master, **kwargs):
        ...
        # expense tabs
        self.expenseBTN = self.createTabButton(
            text="Expense", command=self.onClickExpenseTab
        )
        self.expenseBTN.grid(row=0, column=0, padx=(0, 0))

        # savings tabs
        self.savingsBTN = self.createTabButton(
            text="Savings", command=self.onClickSavingsTab
        )
        self.savingsBTN.grid(row=0, column=1, padx=(BaseStyles.PAD_3, 0))

        # investment tabs
        self.investmentBTN = self.createTabButton(
            text="Investment", command=self.onClickInvestmentTab
        )
        self.investmentBTN.grid(row=0, column=2, padx=(BaseStyles.PAD_3, 0))

        # income tabs
        self.incomeBTN = self.createTabButton(
            text="Income", command=self.onClickIncomeTab
        )
        self.incomeBTN.grid(row=0, column=3, padx=(BaseStyles.PAD_3, 0))

        # all tabs
        self.tab_btns = {
            "expense": self.expenseBTN,
            "savings": self.savingsBTN,
            "investment": self.investmentBTN,
            "income": self.incomeBTN,
        }
