...
from frontend.widgets.table import TransactionTableHeader, TransactionTableBody
...
class HomePageView(View):
    ...
    def _load_icon(self):
        ...
        if hasattr(sys, "_MEIPASS"):  # # for .exe: memory resources path
            ...
            ICONS_FOLDER = os.path.join(_MEIPASS, "frontend\\assets\\icons")
...
class Header(ctk.CTkFrame):
    ...
    def _create_icon(self, img: ctk.CTkImage):
        ...
        self.img_bg.grid(
            row=0, column=1, pady=BaseStyles.PAD_3, padx=BaseStyles.PAD_3, sticky="e"
        )
        self.update_idletasks()
    ...
    def _create_balance(self, summary_type: str, amount: float):
        ...
        self.balance_frame = ctk.CTkFrame(
            master=self, corner_radius=0, fg_color=HomeStyles.BALANCE_FRAME_FG_COLOR
        )
...
class RecentTable(ctk.CTkFrame):
    def __init__(
        self, transactions_per_filter: Dict[str, List[Transaction]], master, **kwargs
    ):
        super().__init__(master, **kwargs)
        # initialize font
        self.font4 = ("Arial", BaseStyles.FONT_SIZE_4, "normal")
        ...
        self.create(
            title_master=master, transactions_per_filter=transactions_per_filter
        )

    def create(
        self, title_master, transactions_per_filter: Dict[str, List[Transaction]]
    ):
...
class MonthlyReport(ctk.CTkFrame):
    ...
    def _create_graphs(self):
        ...
        self.graphs_section = ctk.CTkFrame(
            master=self, fg_color=HomeStyles.MONTHLY_GRAPHS_SECTION_FG_COLOR
        )
    ...
    def display_graphs_section(self) -> Tuple[FigureCanvasTkAgg, FigureCanvasTkAgg]:
        ...
        expense_canvas = FigureCanvasTkAgg(
            figure=expense_graph, master=self.expense_frame
        )
        expense_canvas.draw()
        expense_canvas.get_tk_widget().pack(
            padx=BaseStyles.PAD_1, pady=BaseStyles.PAD_1
        )
        self.update_idletasks()
