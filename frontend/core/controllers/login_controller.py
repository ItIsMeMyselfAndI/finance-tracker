...
from frontend.styles import LoginStyles  # paddings, dimensions, colors, etc
from frontend.core.models import LoginPageModel
from frontend.core.views import LoginPageView
...
class LoginPageController(Controller[LoginPageModel, LoginPageView]):
    def __init__(
        self,
        ...
        self.view = LoginPageView(
            model=self.model,
            page_fg_color=page_fg_color,
            form_fg_color=form_fg_color,
            corner_radius=corner_radius,
            master=master,
        )

    @property
    def model(self) -> LoginPageModel:
        return self.__model

    @model.setter
    def model(self, value: LoginPageModel):
        self.__model = value

    @property
    def view(self) -> LoginPageView:
        return self.__view

    @view.setter
    def view(self, value: LoginPageView):
        self.__view = value
    ...
    def on_click_login(self):
        ...
        if not (account.username and account.password):
            messagebox.showwarning(
                title="[Invalid] Input", message="Empty field is not allowed"
            )
            ...
            messagebox.showwarning(
                title="[DB] No Match Found", message="Incorrect Username or Password"
            )
    ...
    def on_click_signup(self):
        ...
        if not (account.username and account.password):
            ...
            messagebox.showwarning(
                title="[Invalid] Input", message="Username is already taken"
            )
