...
# our modules/libs
# from views.widgets import PopUpWin
...
class SubmitBTN(ctk.CTkButton):
    def __init__(
        self,
        controller_per_page: Dict[str, Controller],
        updating_popup,
        master,
        **kwargs,
    ):
    ...
    def _update_db_and_gui(self):
        ...
        for page_name, controller in self.controller_per_page.items():
            try:
                if page_name == "edit" and controller.model.is_current_page == True:
                    controller.model.save_user_inputs_to_database(
                        controller.view.form_per_transaction_type
                    )
                ...
                messagebox.showwarning(
                    title="[Invalid] Input",
                    message="Only enter positive decimal number for amount",
                )
