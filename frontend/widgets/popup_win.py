...
class PopUpWin(ctk.CTkToplevel):
    ...
    def create_message(
        self, font: tuple, msg: str, text_color: str, width: int, height: int, master
    ):
    ...
    def showWin(self):
        if self.enable_frame_blocker:
            self.input_blocker_frame.place(
                relx=0.0, rely=0.0
            )  # cover screen w/ transparent frame
        self.deiconify()  # show win

    def hideWin(self):
        if self.enable_frame_blocker:
            self.after(
                100, self.input_blocker_frame.place_forget
            )  # uncover screen w/ transparent frame
